...
// переключение между режимом удержания скорости
...
void loop() {
  ...
  if (millis() - tmr >= 50) {
    ...
    if (success) {
      ...
      if (!currentMode) {
        // перевод диапазона 0..255 в -MAX_SPEED..MAX_SPEED
  ...
  // включаем управление скоростью стиками
